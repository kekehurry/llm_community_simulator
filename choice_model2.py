...
class ChoiceModel:
    ...
    def _build_index(self, graph):
        node_embeddings = np.array([node['embedding']
    ...
    def get_period(self, period):
        related_links = [link for link in self._graph.edges(
        ) if self._graph.edges[link]["period"] == period]
        graph = self._graph.edge_subgraph(related_links).copy()
        # related_nodes = [node for node in self._graph.nodes(
        # ) if self._graph.nodes[node]["period"] == period]
    ...
    def similarity_search(self, query, k1=10, k2=5, node_type='Actors'):
        ...
        def social_similarity_search(node_id, k2, node_type='Actors'):
            ...
            same_type_nodes = [node for node in self._graph.nodes(
            ) if self._graph.nodes[node]["type"] == node_type]
            node_degree_centrality = self.degree_centralities[node_id]
            node_degree_centralities = {k: abs(
                v-node_degree_centrality) for k, v in self.degree_centralities.items() if k in same_type_nodes}
            sorted_nodes = sorted(
                node_degree_centralities.items(), key=lambda x: x[1])
        ...
        nodes = []
        scores = []
        senmantic_similar_nodes, senmantic_similar_scores = semantic_similarity_search(
            query, k1)
        for node1, score1 in zip(senmantic_similar_nodes, senmantic_similar_scores):
            social_similar_nodes, social_similar_scores = social_similarity_search(
                node1, k2, node_type)
            filtered_nodes = [
                n for n in social_similar_nodes if n not in nodes]
            filtered_scores = [score1+score2 for score2, n in zip(
                social_similar_scores, social_similar_nodes) if n not in nodes]
            nodes.extend(filtered_nodes)
            scores.extend(filtered_scores)
        return nodes, scores

    def link_prediction(self, profile, k1=10, k2=5, node_type='Actors', choice_type=None):
        ...
        def get_recommendation(G, choice_type=None):
            agents = [n for n, d in G.nodes(data=True) if d['type'] == 'Agent']
            # number of organizations is limited
            if choice_type is None:
                choices = [n for n, d in G.nodes(
                    data=True) if d['type'] != 'Agent']
            else:
                choices = [n for n, d in G.nodes(
                    data=True) if d['type'] in choice_type]
            recommendation = {}
            for choice in choices:
                recommendation[choice] = cal_weighted_jaccard_coeff(
                    G, agents[0], choice)
            recommendation = {k: v for k, v in sorted(
                recommendation.items(), key=lambda item: item[1], reverse=True)}
            choice_ids = list(recommendation.keys())
            choices = [G.nodes[n] for n in choice_ids]
            probabilities = list(recommendation.values())
            choices = choices
            choice_ids = choice_ids
            return choices, probabilities, choice_ids

        # step 1: find nodes with similar profile
        similar_nodes, similar_scores = self.similarity_search(
            profile, k1, k2, node_type)
        ...
        for node, scocre in zip(similar_nodes, similar_scores):
            compute_graph.add_edge(
                agent_id, node, weight=scocre, type='similar_to', label='similar_to')

        # step 5: get recommendation
        choices, probabilities, choice_ids = get_recommendation(
            compute_graph, choice_type)

        return choices, probabilities, choice_ids

    def choose_from_link_prediction(self, profile, k1=10, k2=5, node_type='Actors', choice_type=None, top_k=None):
        choices, probabilities, _ = self.link_prediction(
            profile, k1, k2, node_type, choice_type)
    ...
    def get_old_context(self, profile, k1=10, k2=5, node_type='Actors', choice_type=None, top_k=None):
        choices, probabilities, _ = self.link_prediction(
            profile, k1, k2, node_type, choice_type)
        choices = random.choices(choices, weights=probabilities, k=top_k)
        old_options = [c['properties'] for c in choices]
        options_probabilities = [p for p in probabilities]
        old_context = {f"option_{idx}:{o}" for idx,
                       o in enumerate(old_options)}
    ...
    def get_llm_choice(self, profile, new_options, old_context=None, node_type='Actors', choice_type=None, k1=10, k2=5, top_k=3):
        ...
        if not old_context:
            old_context, old_choices = self.get_old_context(
                profile, k1, k2, node_type, choice_type, top_k)
    ...
    def predict_links(self, test_graph, new_nodes, save_interval=50, file_name="test_period.pkl", k1=10, k2=5, top_k=5, choice_type=None, period=None):
        ...
        for idx, node in tqdm(enumerate(new_nodes), total=len(new_nodes)):
            try:
                ...
                if (len(list(test_graph.nodes))) == 0:
                    ...
                    continue
                old_context, choices = self.get_old_context(
                    profile=profile, node_type=node_type, k1=k1, k2=k2, top_k=top_k)
                ...
                short_term_types = [
                    'Short-term Project', 'Event', 'Media Content', 'Organization']
                long_term_types = ['Long-term Project', 'Actors', 'Space']

                long_term_options = [test_graph.nodes[n]['properties']
                                     for n in link_options if test_graph.nodes[n]['type'] in long_term_types]
                # only consider the short-term nodes in the same period
                short_term_options = [test_graph.nodes[n]['properties'] for n in link_options if (
                    test_graph.nodes[n]['type'] in short_term_types and test_graph.nodes[n]['period'] == period)]

                new_options = long_term_options + short_term_options

                # ask llm to predict links
                llm_choice, llm_response = self.get_llm_choice(
                    profile=profile, new_options=new_options, old_context=old_context, k1=k1, k2=k2, node_type=node_type, choice_type=choice_type, top_k=top_k)
                # add edges
                ...
                for answer, idx in zip(llm_answer, link_options):
                    ...
                    if 'Active' in answer:
                        test_graph.add_edge(
                            node, idx, weight=1, type='connected_to', label='connected_to')
                    elif 'Passive' in answer:
                        test_graph.add_edge(
                            idx, node, weight=1, type='connected_to', label='connected_to')
                        related_actors = [n for n in test_graph.neighbors(
                            idx) if test_graph.nodes[n]['type'] == 'Actors']
                        related_actors = sorted(
                            related_actors, key=lambda n: test_graph.degree(n), reverse=True)
                        if test_graph.nodes[idx]['type'] != 'Actors' and len(related_actors) > 0:
                            organizer = related_actors[0]
                            test_graph.add_edge(
                                organizer, node, weight=1/test_graph.degree(organizer), type='connected_to', label='connected_to')
                        else:
                            test_graph.add_edge(
                                idx, node, weight=1/test_graph.degree(idx), type='connected_to', label='connected_to')
                    elif 'No' in answer:
                        pass
                    else:
                        print(answer)
    ...
    def evaluate(self, graph=None):
        if graph is None:
            graph = self.graph
        actors = [n for n in graph.nodes() if graph.nodes[n]
                  ['type'] == 'Actors']
        ...
        def cal_CPR():
            CPR = np.mean([cal_single_PPI(a) for a in actors])
            return CPR

        def cal_ACI():
            ACI = np.mean([graph.degree(a) for a in actors])
            return ACI
        # def cal_CVI():
        #     cluster = nx.clustering(graph)
        #     CVI = np.mean([cluster[a] for a in actors])
        #     return CVI
