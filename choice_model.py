...
class ChoiceModel:
    ...
    def set_period(self, period=None):
        ...
        node_embeddings = np.array([node['embedding'] for idx,node in self.graph.nodes(data=True)])
        index = faiss.IndexFlatL2(self.embedding_dimension)
        index.add(node_embeddings)
        ids = np.array([node for node in self.graph.nodes()])
    ...
    def similarity_search(self,query, k1=5, k2=2,node_type='Actors'):
        ...
        def social_similarity_search(node_id,k2,node_type='Actors'):
            ...
            same_type_nodes = [node for node in self.graph.nodes() if self.graph.nodes[node]["type"] == node_type]
    ...
    def link_prediction(self, profile, k1, k2, node_type='Actors', choice_type='Actors'):
        ...
        def get_recommendation(G,choice_type):
            ...
            probabilities = softmax(list(recommendation.values())).tolist()
        ...
        for n in similar_nodes:
            neighbors.extend(list(self.graph.neighbors(n)))
            neighbors.append(n) 

        # step 3: create compute graph
        compute_graph = nx.subgraph_view(self.graph, filter_node=lambda n: n in neighbors).copy()
